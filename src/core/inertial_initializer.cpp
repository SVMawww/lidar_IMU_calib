...
bool InertialInitializer::estimateRotation(
        TrajManager<4>::Ptr traj_manager,
        const Eigen::aligned_vector<LiDAROdometry::OdomData>& odom_data) {
  auto p_traj = traj_manager->getTrajPtr();
  Eigen::aligned_vector<Eigen::Matrix4d> A_vec;
  for (size_t j = 1; j < odom_data.size(); ++j) {
    size_t i = j - 1;
    double ti = odom_data.at(i).timestamp;
    ...
    if (tj >= p_traj->maxTime())
      break;
    auto result_i = p_traj->EvaluatePose(ti);
    auto result_j = p_traj->EvaluatePose(tj);
    Eigen::Quaterniond delta_qij_imu = result_i.so3().unit_quaternion().conjugate()
                                       * result_j.so3().unit_quaternion();
